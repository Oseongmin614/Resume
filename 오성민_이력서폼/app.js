...
class TypingEffect {
  constructor(element, options = {}) {
    ...
    this.options = {
      speed: options.speed || CONFIG.TYPING_SPEED, // 각 줄이 나타나는 속도
      delay: options.delay || 500,
      onComplete: options.onComplete || (() => {}), // 완료 콜백 추가
      ...options
    };
    this.init();
  }
  ...
  init() {
    ...
    // innerHTML을 가져와 <br> 태그를 포함한 원본 내용을 보존
    const originalHTML = this.element.innerHTML;
    this.element.innerHTML = ''; // 내용을 비움
    
    // <br> 태그를 기준으로 줄을 분리
    const lines = originalHTML.split(/<br\s*\/?>/i).map(line => line.trim());
  ...
  typeLines(lines, index) {
    if (index < lines.length) {
      // 현재 줄을 추가
      this.element.innerHTML += lines[index];
...
class ResumeApp {
  ...
  initializeComponents() {
    try {
      ...
      const summaryElement = Utils.$('.summary-text');
      if (summaryElement) {
        this.components.typingEffect = new TypingEffect(summaryElement, {
          onComplete: () => {
            if (this.components.navigation) {
              this.components.navigation.updateActiveLink();
            }
          }
        });
      }
